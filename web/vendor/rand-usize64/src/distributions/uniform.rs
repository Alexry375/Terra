...
// ---------------------------------------------------------------------------
// (terra / interface-harnais) LE SEUL ÉCART AVEC `rand` 0.8.7 AMONT.
//
// En amont, `usize` s'échantillonne avec sa propre largeur : 64 bits sur un
// ordinateur 64 bits, 32 bits en WebAssembly. Un `rng.gen_range(0..n)` sur un
// `usize` ne consomme donc pas le même nombre d'octets du générateur, et ne
// rend pas la même valeur, selon la cible — alors que l'état du générateur, lui,
// est identique.
//
// Le moteur Terra prend TOUTES ses décisions par `gen_range` sur des `usize`
// (indices d'options, mélanges de Fisher-Yates). Compilé en WebAssembly sans
// cette correction, il répond différemment du binaire natif à des questions
// pourtant déterministes — par exemple `--probe` sur les 19 cartes qui
// accordent une carte Phase améliorée.
//
// Correction : sur une cible de moins de 64 bits, `usize` s'échantillonne
// EXACTEMENT comme `u64` (même largeur de tirage, même arithmétique de rejet).
// Sur une cible 64 bits, `usize` EST `u64` : l'instanciation d'amont est donc
// conservée telle quelle, et rien ne change pour elle.
//
// Limite connue et sans effet ici : l'intervalle PLEIN (`0..=usize::MAX`) tire
// alors 32 bits au lieu de 64. Le moteur ne l'emploie jamais — ses intervalles
// sont des tailles de listes d'options.
#[cfg(target_pointer_width = "64")]
uniform_int_impl! { usize, usize, usize }
#[cfg(not(target_pointer_width = "64"))]
uniform_int_impl! { usize, u64, u64 }
